...
Parser.prototype.parse = function(str, options) {
  ...
  for (var i = dataArray.length - 1; i >= 0; i--) {
    for (var j = dataArray[i].length - 1; j >= 0; j--) {
      if(opts.ignoreMissingColumns) {
        dataArray[i][j] = dataArray[i][j] && dataArray[i][j].replace(/([\t\n])/, '\\$1');
      } else {
  ...
  for (var ii = 0; ii < dataArray.length; ii++) {
    ...
    if (opts.ignoreMissingColumns && (numValues > numColumns) ||
        !opts.ignoreMissingColumns && (numValues != numColumns)) {
